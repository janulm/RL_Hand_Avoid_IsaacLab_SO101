...
class ObjCameraGrayDepthPoseTrackingDirectEnvCfg(DirectRLEnvCfg):
    ...
    table_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path=str(ASSET_DIR / "table.usd"),
    ...
    arm_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path=str(ASSET_DIR / "arm.usd"),
    ...
    i2r_plane_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path=str(ASSET_DIR / "i2r_plane.usd"),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                rigid_body_enabled=True,
                kinematic_enabled=True,
                disable_gravity=True,
    ...
    clemson_plane_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path=str(ASSET_DIR / "clemson_plane.usd"),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                rigid_body_enabled=True,
                kinematic_enabled=True,
                disable_gravity=True,
    ...
    curriculum_target_ranges = [
        {"x": (0.30, 0.46), "y": (0.62, 0.70), "z": (-0.1, 0.1)},  # Easy
        {"x": (0.28, 0.52), "y": (0.62, 0.74), "z": (-0.15, 0.1)},  # Medium
        {"x": (0.26, 0.58), "y": (0.60, 0.78), "z": (-0.2, 0.1)},  # Hard
        {"x": (0.24, 0.62), "y": (0.58, 0.82), "z": (-0.2, 0.1)},  # Expert
    ]
    ...
    target_pose_range = {
        "x": (0.28, 0.55),
        "y": (0.62, 0.78),
        "z": (-0.7, -0.5),  # wrt base link of robot [-80mm to +150mm] irl
        "roll": (0.0, 0.0),
        "pitch": (1.57, 1.57),
        "yaw": (0.0, 0.0),
    }
    target_max_radius = 0.82
    target_arm_clearance = 0.12
    target_resample_attempts = 16
    reset_arm_motion_pattern = None  # None keeps random arm motion patterns
    ...
    # Reward settings
    reward_distance_weight = -3.5
    reward_distance_tanh_weight = 2.5
    ...
    reward_arm_avoidance_weight = 3.0  # Changed from obstacle
    reward_action_rate_weight = -1.0  # Increased penalty for jagged movements

    # Artificial Potential Field parameters
    apf_critical_distance = 0.10  # db - critical distance for obstacle avoidance
    apf_smoothness = 0.1  # ko - smoothness parameter for beta transition
    apf_min_task_blend = 0.35  # Always keep some task reward near the arm
...
class ObjCameraGrayDepthPoseTrackingDirectEnv(DirectRLEnv):
    ...
    def _sample_commands(self, env_ids: Sequence[int]) -> None:
        ...
        x, y, z = self._resample_targets_for_reach_and_clearance(env_ids, x, y, z)
    ...
    def _resample_targets_for_reach_and_clearance(
        self,
        ...
        """Reject targets that are outside reach or already occupied by the arm."""
        max_radius = float(getattr(self.cfg, "target_max_radius", float("inf")))
        min_arm_clearance = float(getattr(self.cfg, "target_arm_clearance", 0.0))
        max_attempts = int(getattr(self.cfg, "target_resample_attempts", 0))
        if max_attempts <= 0:
            return x, y, z
        ...
        for _ in range(max_attempts):
            target_pos_b = torch.stack([x, y, z], dim=-1)
            radius_invalid = torch.norm(target_pos_b[:, :2], dim=-1) > max_radius
            clearance_invalid = torch.zeros_like(radius_invalid)

            if min_arm_clearance > 0.0 and hasattr(self, "_arm"):
                robot_pos = self._robot.data.root_state_w[env_ids_t, :3]
                robot_quat = self._robot.data.root_state_w[env_ids_t, 3:7]
                target_pos_w, _ = math_utils.combine_frame_transforms(
                    robot_pos, robot_quat, target_pos_b
                )
                arm_distances = self._point_to_box_distance(
                    target_pos_w,
                    self._arm.data.root_pos_w[env_ids_t, :3],
                    self._arm.data.root_quat_w[env_ids_t],
                    arm_half_extents,
                )
                clearance_invalid = arm_distances < min_arm_clearance

            needs_resample = radius_invalid | clearance_invalid
            if not torch.any(needs_resample):
                break

            count = int(needs_resample.sum().item())
            x[needs_resample] = sample_uniform(
                self.cfg.target_pose_range["x"][0],
                self.cfg.target_pose_range["x"][1],
                (count,),
                self.device,
            )
            y[needs_resample] = sample_uniform(
                self.cfg.target_pose_range["y"][0],
                self.cfg.target_pose_range["y"][1],
                (count,),
                self.device,
            )
            z[needs_resample] = sample_uniform(
                self.cfg.target_pose_range["z"][0],
                self.cfg.target_pose_range["z"][1],
                (count,),
                self.device,
    ...
    def _compute_beta_transition(self, min_distances: torch.Tensor) -> torch.Tensor:
        ...
        return torch.clamp(beta, min=self.cfg.apf_min_task_blend)
    ...
    def _point_to_box_distance(
        ...
        distances = torch.zeros(points.shape[0], device=self.device)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        arm_collision = min_distances_to_arm < self.cfg.apf_critical_distance * 0.25

        # Early termination combines time out and bounds violation
        early_termination = time_out
        return task_success | arm_collision, early_termination
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        reset_motion_pattern = getattr(self.cfg, "reset_arm_motion_pattern", None)
        if reset_motion_pattern is not None:
            reset_ids = torch.as_tensor(env_ids, device=self.device, dtype=torch.long)
            if not hasattr(self, "_arm_motion_time"):
                self._arm_motion_time = torch.zeros(self.num_envs, device=self.device)
            if not hasattr(self, "_arm_motion_pattern"):
                self._arm_motion_pattern = torch.zeros(
            ...
            self._arm_motion_time[reset_ids] = 0.0
            self._arm_motion_pattern[reset_ids] = int(reset_motion_pattern)
            self._arm_target_pos[reset_ids, 0] = base_x - amp_x
            self._arm_target_pos[reset_ids, 1] = base_y
            self._arm_target_pos[reset_ids, 2] = base_z
