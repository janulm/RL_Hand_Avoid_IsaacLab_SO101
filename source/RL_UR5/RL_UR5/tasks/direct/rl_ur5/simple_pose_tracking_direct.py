...
class SimpleCameraPoseTrackingEnvCfg(DirectRLEnvCfg):
    ...
    observation_space = gym.spaces.Dict(
        {
            ...
            #     low=float("-inf"), high=float("inf"), shape=(3, 100, 80)
            # ),
    ...
    tiled_camera: TiledCameraCfg = TiledCameraCfg(
        ...
        offset=TiledCameraCfg.OffsetCfg(
            pos=(1.5, 0.5, 1.5), rot=(0.679, 0.281, 0.281, 0.613), convention="opengl"
        ),
    )
    ...
    # Reward weights
    position_reward_weight = -10.0  # Weight for Huber loss (penalty)
    orientation_reward_weight = -5.0  # Weight for orientation error (penalty)
    action_rate_penalty_weight = -0.1  # Weight for action rate (penalty)
    success_reward = 100.0  # Bonus for success
...
class SimpleCameraPoseTrackingEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Logging
        self._episode_sums = {
            "position_error": torch.zeros(
                self.num_envs, dtype=torch.float, device=self.device
            ),
            "orientation_error": torch.zeros(
                self.num_envs, dtype=torch.float, device=self.device
            ),
        ...
        print(
            f"[INFO] Simple Camera Pose Tracking Environment initialized with {self.num_envs} environments"
        )
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # Note: We update _previous_actions AFTER reward calculation or at start of episode?
        # Actually it's best to store it here for the NEXT step's pre-physics, but for reward calculation we need (current - prev).
        # So we should validly use self._previous_actions in get_rewards, THEN update it.
        # But this _pre_physics_step is called at START of step.
        # So if we overwrite here, we lose the old one. We should NOT overwrite here.
        # We will update _previous_actions in a post-step or simply at the end of _get_rewards (hacky)
        # or we rely on the fact that we can store `self._previous_actions` and update it at the END of step.

        # Let's remove the overwrite here. We'll update it in `_get_rewards` or `_post_physics_step` if possible.
        # Since DirectRLEnv doesn't expose post_physics easily without override, let's update it in get_rewards?
        # A safer pattern is:
        # 1. get_rewards uses (self._actions, self._previous_actions)
        # 2. We update self._previous_actions = self._actions.clone() at the VERY END of get_rewards.
        pass
    ...
    def _apply_action(self):
        ...
        self._robot_dof_targets = self._actions * self.cfg.action_scale
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Huber loss for position error
        # Delta = 0.1 for Huber loss
        delta = 0.1
        huber_loss = torch.where(
            position_error < delta,
            0.5 * position_error**2,
            delta * (position_error - 0.5 * delta),
        )
        position_reward = (
            huber_loss * self.cfg.position_reward_weight
        )  # Negative weight

        # Orientation tracking reward (penalty)
        orientation_error = math_utils.quat_error_magnitude(ee_quat, desired_quat_w)
        orientation_reward = (
            orientation_error * self.cfg.orientation_reward_weight
        )  # Negative weight

        # Action rate penalty
        current_actions = self._actions
        action_rate = torch.norm(current_actions - self._previous_actions, p=2, dim=1)
        action_rate_penalty = (
            action_rate * self.cfg.action_rate_penalty_weight
        )  # Negative weight

        # Store current actions as previous (should happen in pre_physics or apply_action, but works here if rewards calc is end of step)
        # Actually _pre_physics_step handles updating _previous_actions from _actions if we did it right.
        # But wait, _pre_physics_step: self._previous_actions = self._actions.clone() sets them to be SAME.
        # So action_rate would be ZERO if we don't track the *actual* previous steps.
        # We need to ensure _previous_actions holds the PREVIOUS step's actions.
        # Let's fix that logic in a separate MultiReplace check or rely on proper Step handling.
        # DirectRLEnv structure usually: pre_physics -> step -> post_phys -> get_obs -> get_rewards.
        # In _pre_physics_step, we normally *apply* actions.
        # If I overwrite `self._previous_actions` with current `actions` in `_pre_physics_step`, I lose history for this step's reward calc.

        # Success reward
        is_success = (position_error < self.cfg.position_threshold) & (
            orientation_error < self.cfg.orientation_threshold
        )
        success_reward = is_success.float() * self.cfg.success_reward

        # Total reward
        rewards = (
            position_reward + orientation_reward + action_rate_penalty + success_reward
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        success = (position_error < self.cfg.position_threshold) & (
            orientation_error < self.cfg.orientation_threshold
        )
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if len(env_ids) > 0:
            ...
            for key, value in self._episode_sums.items():
                if key.endswith("_error"):
                    ...
                    extras[f"Episode/{key}"] = torch.mean(
                        value[env_ids] / self.episode_length_buf[env_ids]
                    ).item()
                else:
                    # Total reward
        ...
        noise = torch.zeros_like(joint_pos).uniform_(
            -self.cfg.robot_reset_noise, self.cfg.robot_reset_noise
        )
        joint_pos += noise
    ...
    def _sample_target_poses(self, env_ids: Sequence[int]):
        ...
        self._desired_pos_b[env_ids, 0] = torch.zeros_like(
            self._desired_pos_b[env_ids, 0]
        ).uniform_(self.cfg.position_range["x"][0], self.cfg.position_range["x"][1])
        self._desired_pos_b[env_ids, 1] = torch.zeros_like(
            self._desired_pos_b[env_ids, 1]
        ).uniform_(self.cfg.position_range["y"][0], self.cfg.position_range["y"][1])
        self._desired_pos_b[env_ids, 2] = torch.zeros_like(
            self._desired_pos_b[env_ids, 2]
        ).uniform_(self.cfg.position_range["z"][0], self.cfg.position_range["z"][1])

        # Sample orientations using the same pattern
        roll = torch.zeros(len(env_ids), device=self.device).uniform_(
            self.cfg.orientation_range["roll"][0], self.cfg.orientation_range["roll"][1]
        )
        ...
        yaw = torch.zeros(len(env_ids), device=self.device).uniform_(
            self.cfg.orientation_range["yaw"][0], self.cfg.orientation_range["yaw"][1]
        )
